...
def main():
    ...
    for s in zip(corrpaths,datapaths):
      ...
      print(Corrs)
      C2 = ana.read_confs(s[0]+"/C2+_cnfg",Corrs[2][0],conf_feed_stringlist,
                          T,h5=True,verb=False)
      print("C4")
      print("Reading Correlation functions from %s..." % s[0])
      C4D = ana.read_confs(s[0]+"/C4+D_cnfg" ,Corrs[1][0],conf_feed_stringlist,
                           T,h5=True,verb=True)
      C4C = ana.read_confs(s[0]+"/C4+C_cnfg",Corrs[0][0],conf_feed_stringlist,
                           T,h5=True)
      print("Read in done")
    ...
    if c2_pi:
        ...
        # copy common subset of configurations to appropriate target directory:
        # Read in correlators
        print("Reading Pion Correlation functions from %s..." % corrpaths_pi[0])
        print("C2")
        ...
        #  C2[i] = _Cfull
        C2 = ana.read_confs(corrpaths_pi[0]+"/C2+_cnfg",Corrs_pi[3][0],conf_feed_stringlist,T,h5=True,verb=True)
        print("Read in done")
        save_pi = datadir+'/pi/'

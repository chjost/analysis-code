...
def main():
    ...
    for s in zip(corrpaths,datapaths):
      ...
      print("C2")
      C2 = ana.read_confs(s[0]+"/C2+_cnfg",Corrs[0],conf_feed_stringlist,T,h5=True)
      print("C4")
      C4D = ana.read_confs(s[0]+"/C4+D_cnfg" ,Corrs[1],conf_feed_stringlist,T,h5=True)
      C4C = ana.read_confs(s[0]+"/C4+C_cnfg",Corrs[2],conf_feed_stringlist,T,h5=True)
      print("Read in done")

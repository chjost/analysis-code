...
def gamma_errfunc(p,x,y,cov):
    ...
    if y.shape[0] > x.shape[0]:
        ...
        _res = np.r_[x,p[-1]-2*p[-2]*x]-y[:-1]
        #_res = np.r_[_res,p[1]-y[-2],p[2]-y[-1]]
        _res = np.r_[_res,p[-1]-y[-1]]
    else: 
      _res = np.r_[x,p[-1]-2*p[-2]*x]-y
...
################################################################################
    ...
    for i,fr in enumerate(fit_ranges):
        ...
        L5.info()
        # get the ydata
        ydata = get_dataframe_fit_xy(fit_df,'M_K/M_pi','Gamma',['beta','L','mu_l'],
                                  'sample',priors=L5)
        ydata.info()
        print(xdata)
        print(ydata.sample(n=20))
        chi.print_si_format(means)
        data_for_cov = get_dataframe_fit_xy(fit_df,'M_K/M_pi','Gamma',

...
def main():
    ...
    print(masses.data[0][...,0].shape)
    print(addT.shape)
    add = np.hstack((masses.data[0][...,0],addT.reshape((addT.shape[0],1))))
    ...
    # symmetrize and bootstrap
        ...
        pik_corr.save_h5("%s/%s_%s.h5"%(datadir,"corr_pik",lat),'raw')
    else:
    ...
    if do_e1 is True:
        ...
        e1_corr_ws.save_h5("%s/%s_%s.h5" %(datadir,"corr_pik",lat),'ws_e1')
        # fit e1
        print("\nFitting Branch E1")
        # UNCORRELATED FIT
        ...
        pi_k_fitresult_e1.save_h5("%s/%s.h5" % (datadir,"fit_pik"),
                                  'fit_corr_e1_corr_false')
    ...
    if do_e2 is True:
        ...
        e2_corr_ws.save_h5("%s/%s_%s.h5" %(datadir,"corr_pik",lat),'ws_e2')
        ...
        pi_k_fitresult_e2 = fit_pi_k_e2.fit(start,e2_corr_ws,fit_e_tot,add=addT,
                                            oldfit=masses)
        pi_k_fitresult_e2.save_h5("%s/%s.h5" % (datadir,"fit_pik"),
                                  'fit_corr_e2_corr_false')
